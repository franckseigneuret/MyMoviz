...
const App = (props) => {
  ...
  const checkMovieLiked = (isLiked, name, img) => {
    if (isLiked && wishlist.findIndex(item => item.name === name) < 0) { // si on LIKE un film et que wishList ne le contient pas 
      setWishList([...wishlist, {name: name, img: img}])          // on AJOUTE ce film dans la wishList
    } else if (!isLiked && wishlist.findIndex(item => item.name === name) >= 0) {   // si on UN-like un film et que wishList le contient 
